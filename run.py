...
class Run():
    def __init__(self,
        ...
        self.save_dir = './model_save_default'
        ...
        self.flow_lr = config.get('flow_lr', 0.001)
        self.flow_hidden_dim = config.get('flow_hidden_dim', 128)
        self.flow_dropout = config.get('flow_dropout', 0.1)
    ...
    def get_optimizer(self, model, diff_model=None, ss_model=None, la_model=None, flow_model=None):
        ...
        if diff_model is None and ss_model is None and la_model is None and flow_model is None:
            ...
            optimizer_flow = torch.optim.Adam(params=flow_model.parameters(), lr=self.flow_lr)
            return optimizer_src, optimizer_tgt, optimizer_meta, optimizer_aug, optimizer_flow, optimizer_map
    ...
    def eval_mae(self, model, data_loader, stage ):
        ...
        with torch.no_grad():
            ...
            elif stage == 'test_flow':
                for X in tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0):
                    model[0].eval() 
                    model[1].eval() 
                    pred = model[0](X, stage, self.device, flow_model=model[1])
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch, stage, mapping=False, diff=False, ss=False, la=False, flow=False):
        print('Training Epoch {}:'.format(epoch + 1))

        loss_ls = []
        if diff == False and ss==False and la==False and flow==False:
            for X, y in tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0):
                if mapping:
                    model.train()
            ...
            for X in tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0):
                ...
                # flow first (physics), then task (alignment)
                
                # Step 1: Flow Step (is_task=False)
                loss = model[0](X, stage, self.device, flow_model=model[1], is_task=False) 
                model[1].zero_grad()
                loss.backward()
                _ = torch.nn.utils.clip_grad_norm_(model[1].parameters(), 1.)
                optimizer.step()

                # Step 2: Task Step (is_task=True)
                task_loss = model[0](X, stage, self.device, flow_model=model[1], is_task=True)
                model[1].zero_grad()
                task_loss.backward()
                _ = torch.nn.utils.clip_grad_norm_(model[1].parameters(), 1.)
                optimizer.step()
    ...
    def Flow_CDR(self, model, flow_model, data_loader, data_test, optimizer):
        print('=========Flow_CDR========')
        best_mae = float('inf')
        for i in range(self.epoch):
            ...
            mae, rmse = self.eval_mae([model, flow_model], data_test, stage='test_flow')
            self.update_results(mae, rmse, 'flow')

            print(f'FLOW LOSS: {loss.item()} | TASK LOSS: {task_loss.item()} | '
                  f'MAE: {mae} RMSE: {rmse}')
    ...
    def main(self, exp_part = 'None_CDR',save_path=None):
        ...
        elif exp_part == 'flow_CDR':
            flow_model = Flow.DeltaFMCDR(
                input_dim=self.emb_dim,
                hidden_dim=self.config.get('flow_hidden_dim', 128),
                dropout=self.config.get('flow_dropout', 0.1)
            )
            flow_model = flow_model.cuda() if self.use_cuda else flow_model

            model = self.get_model()

            optimizer_src, optimizer_tgt, optimizer_meta, optimizer_aug, optimizer_flow, optimizer_map = self.get_optimizer(model, flow_model=flow_model)
        ...
        if exp_part == 'None_CDR':
            ...
            self.model_load(model, path=save_path)
            print('None_CDR model loaded for Flow')
            
            self.Flow_CDR(model, flow_model, data_diff, data_diff_test, optimizer_flow)
            
            self.result_print(['flow'])
