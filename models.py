...
class MFBasedModel(torch.nn.Module):
    ...
    def forward(self, x, stage, device, diff_model=None, ss_model=None, la_model=None, flow_model=None, is_task=False):
        if stage == 'train_src':
        ...
        elif stage == 'train_flow':
            tgt_uid, iid_input, y_input = x

            u_src = self.src_model.uid_embedding(tgt_uid.unsqueeze(1)).squeeze()
            u_tgt = self.tgt_model.uid_embedding(tgt_uid.unsqueeze(1)).squeeze()
            i_tgt = self.tgt_model.iid_embedding(iid_input.unsqueeze(1)).squeeze()
            
            loss = flow_model.calculate_loss(u_src, u_tgt, i_tgt, y_input, is_task=is_task)
            return loss
            ...
            u_src = self.src_model.uid_embedding(tgt_uid.unsqueeze(1)).squeeze()
            u_mapped = flow_model.map_user(u_src)
            
            i_tgt = self.tgt_model.iid_embedding(iid_input.unsqueeze(1)).squeeze()
            
            score = torch.sum(u_mapped * i_tgt, dim=1)
            return score
